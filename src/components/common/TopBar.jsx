...
export default function TopBar({setOpenChat, stack, difficulty, questions}){
    return(<>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <span className="md:text-xl">{stack.icon}</span>
          <span className="md:text-xl font-bold text-white" >{stack.label}</span>
          <span className={`text-xs md:text-sm font-semibold px-3 py-1 rounded-2xl ${difficulty === "junior" ? "bg-green-500/10" : difficulty === "mid" ? "bg-yellow-500/10" : "bg-red-500/10"} 
