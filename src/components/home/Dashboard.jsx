...
export default function Dashboard(){
    ...
    return(<>
        <div className="m-auto p-5 md:p-10"> 
            ...
            <div className="flex gap-4 md:gap-30 my-8">
            {[
                ...
                <motion.div initial={{ y:40, opacity: 0}} transition={{duration: 0.5, delay: i*0.2,  ease: "easeInOut"}} animate={{y: 0, opacity: 1}}
                            key={i} className={` bg-gray-800/30 border-t border-b rounded-2xl ${borderColors[stat.color]} p-2 md:p-10 w-25 m-auto md:w-100`}>
                <div className=" text-xl md:text-4xl mb-4" >{stat.icon}</div>
                <div className=" text-xl md:text-4xl font-bold" style={{ color: stat.color }}>{stat.value}</div>
                <div className=" text-xs md:text-xl mt-2" style={{ color: "#8B949E"}}>{stat.label}</div>
                </motion.div>
            ))}
            </div>
        </div>
